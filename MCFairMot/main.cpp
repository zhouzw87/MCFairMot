...
void DrawTracks(cv::Mat&img, std::string cls, std::vector<std::shared_ptr<STrack>>& tracks)
{
	for (auto& tr:tracks)
	{
		...
		double fontScale = 0.6; //0.6; //1.2f;
		int fontThickness = 1; //1; // 2;
		char text[15];
		sprintf(text, "%d", tr->GetTrackID());
		std::string buff = text;
		putText(img, cls+":"+buff, cv::Point((int)loc.x, (int)loc.y), fontCalibration, fontScale, color, fontThickness);/**/
		if (tr->GetFrameID() - tr->GetStartFrame()>60) {
			//std::cout << "TrackID:"   << tr->GetTrackID() <<"   TrackTime: " << tr->GetFrameID() - tr->GetStartFrame() << std::endl;
			for (auto rt : tr->GetTrajectory())
			{
				cv::circle(img, cv::Point(rt.x+rt.width/2, rt.y+rt.height/2), 1, color, 1);
			}
		}
	}
}

std::string GetClassIndex(int cls) {
	std::string str_cls = "";
	if (cls == 0) {
		str_cls = "car";
	}
...
void main()
{
	...
	while (true)
	{
		...
		double inference_fps = 1000.0 / std::chrono::duration_cast<std::chrono::milliseconds>(track_end - detect_start).count();
		std::cout << "detection:" << detection_time << ", tracking:" << tracking_time << ", fps:" << inference_fps << std::endl;
		for (int cls = 0; cls < num_class; cls++) {
			std::string cls_str = GetClassIndex(cls);
			DrawTracks(frame, cls_str, tracks[cls]);
		}
